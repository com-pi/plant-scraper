...
def scrape_detail_new_version(soup) -> PlantDetails:
    ...
    name = soup.select_one("#plantDetail__page > div > div > div > section.plantDetail__section > div > h1").text.strip()
    description = soup.select_one("#plantDetail__page > div > div > div > section.plants-simpleinfo-section > p").text.strip()
    info: List[PlantAttribute] = []
    col = ["물주기", "광도", "습도", "온도"]
